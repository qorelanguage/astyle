...
const char* g_version = "3.1.1q beta";
...
//  TODO - work around --mode=qore
//    else if (stringEndsWith(fileName_, string(".q*")))
//      formatter.setQoreStyle();
		else
			formatter.setCStyle();
	}
	...
	while (formatter.hasMoreLines())
	{
		...
		else
		{
			...
			// this can happen if the file is missing a closing brace and break-blocks is requested
			if (formatter.getIsLineReady())
			{
				setOutputEOL(lineEndFormat, streamIterator.getOutputEOL());
				out << outputEOL;
...
{
	cout << endl;
	cout << "                     Artistic Style " << g_version << " with Qore alpha support " << endl;
	...
	cout << endl;
	cout << "    --mode=qore\n";
	cout << "    Indent a qore source file.\n";
	cout << endl;
...
void ASOptions::parseOption(const string& arg, const string& errorInfo)
{
  ...
  else if (isOption(arg, "mode=qore"))
  {
		formatter.setQoreStyle();
		formatter.setModeManuallySet(true);
  }
